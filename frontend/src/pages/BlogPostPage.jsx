...
export function BlogPostPage({ slug }) {
  ...
  const translatedPost = useRuntimeTranslatedValue(post)
  const translatedComments = useRuntimeTranslatedValue(comments)

  const parsedContent = useMemo(() => renderContentBlocks(translatedPost?.content || translatedPost?.excerpt || ''), [translatedPost?.content, translatedPost?.excerpt])

  const loadComments = useCallback(async (nextSlug) => {
    ...
    try {
      ...
      setCommentsError(err?.message || t('blogPost.commentError'))
    } finally {
  ...
  useEffect(() => {
    let active = true
    const load = async () => {
      if (!slug) { setError(t('blogPost.invalidLink')); setLoading(false); return }
      try {
        setLoading(true); setError('')
        const next = await publicService.getPostBySlug(slug)
        if (!active) return
        if (!next) { setError(t('blogPost.postNotFound')); setPost(null); return }
        setPost(next)
        await loadComments(slug)
      } catch (err) {
        if (!active) return
        setError(err?.message || t('blogPost.loadError'))
        setPost(null)
      } finally {
  ...
  const submitComment = async (e) => {
    ...
    if (!payload.content) { setCommentsError(t('blogPost.writeFirst')); return }
    setSubmittingComment(true)
    try {
      await publicService.submitPostComment(slug, payload)
      setCommentNotice(t('blogPost.commentSuccess'))
  ...
  const catColors = CATEGORY_COLORS[translatedPost?.category] || { bg: '#1B2E4B', text: '#fff' }
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f5f7fb' }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 20, fontSize: 13 }}>
            <span style={{ color: 'rgba(255,255,255,0.5)', cursor: 'pointer' }} onClick={() => navigateTo('home')}>{t('common.home')}</span>
            <ChevronRight size={13} color="rgba(255,255,255,0.3)" />
            <span style={{ color: 'rgba(255,255,255,0.5)', cursor: 'pointer' }} onClick={() => navigateTo('blog')}>{t('blogPage.breadcrumb')}</span>
            <ChevronRight size={13} color="rgba(255,255,255,0.3)" />
            <span style={{ color: '#FF6B35', fontWeight: 600, maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {translatedPost?.title || t('blogPost.article')}
            </span>
          </div>
          {translatedPost && (
            <div className="fade-up">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, fontSize: 13, color: 'rgba(255,255,255,0.55)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><User size={14} color="#FF6B35" />{translatedPost.author}</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Calendar size={14} color="#FF6B35" />{translatedPost.date}</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Clock size={14} color="#FF6B35" />{translatedPost.readTime}</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Eye size={14} color="#FF6B35" />{(translatedPost.views || 0).toLocaleString()} {t('blogPost.views')}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 24px 64px' }}>
        ...
        {loading && <p style={{ color: '#6b7a90', padding: '48px 0' }}>{t('blogPost.loading')}</p>}
        {!loading && error && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 36, alignItems: 'start', paddingTop: 36 }}>
            ...
            <div>
              ...
              {translatedPost.image && (
                <div style={{ borderRadius: 14, overflow: 'hidden', marginBottom: 36, boxShadow: '0 8px 32px rgba(15,31,61,0.12)' }}>
                  <img src={translatedPost.image} alt={translatedPost.title} style={{ width: '100%', height: '420px', objectFit: 'cover', display: 'block' }} />
                </div>
              )}

              {/* Tags */}
              {Array.isArray(translatedPost.tags) && translatedPost.tags.length > 0 && (
              ...
              <article style={{
                ...
                {parsedContent.length > 0
                  ? parsedContent
                  : <p style={{ color: '#6b7a90', fontSize: 16 }}>{t('blogPost.article')}</p>
                }
              </article>
              ...
              <section style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  <h2 style={{ fontSize: 22, fontWeight: 800, color: '#0f1f3d', margin: 0 }}>{t('blogPost.discussion')}</h2>
                ...
                <p style={{ color: '#8a96a8', fontSize: 13.5, marginBottom: 24 }}>{t('blogPost.commentModerated')}</p>
                ...
                <div style={{ backgroundColor: '#fff', borderRadius: 14, border: '1px solid #e8edf4', padding: '26px 28px', marginBottom: 28, boxShadow: '0 2px 12px rgba(15,31,61,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                    ...
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0f1f3d', margin: 0 }}>{t('blogPost.writeComment')}</h3>
                  </div>
                  <form onSubmit={submitComment} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                      ...
                      >
                        {submittingComment
                          ? <><MessageCircle size={15} /> {t('blogPost.posting')}</>
                          : <><Send size={15} /> {t('blogPost.postComment')}</>
                        }
                      </button>
                    </div>
                  </form>
                ...
                {/* ── Existing Comments ── */}
                {commentsLoading && (
                  <div style={{ padding: '20px 0', color: '#8a96a8', fontSize: 14 }}>{t('blogPost.loadingComments')}</div>
                )}
                {!commentsLoading && translatedComments.length === 0 && !commentsError && (
                ...
                {!commentsLoading && translatedComments.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {translatedComments.map((comment, i) => (
                      <div
                        key={comment.id}
            ...
            <aside style={{ position: 'sticky', top: 'calc(var(--site-header-height) + 24px)', display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {translatedPost.author && (
                <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: '20px 20px', border: '1px solid #e8edf4', textAlign: 'center', boxShadow: '0 2px 10px rgba(15,31,61,0.05)' }}>
                  <div style={{ width: 54, height: 54, borderRadius: '50%', background: 'linear-gradient(135deg, #FF6B35, #1B2E4B)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 800, color: '#fff', margin: '0 auto 12px', letterSpacing: '-0.02em' }}>
                    {translatedPost.author.slice(0, 2).toUpperCase()}
                  </div>
                  <p style={{ fontSize: 15, fontWeight: 700, color: '#0f1f3d', marginBottom: 4 }}>{translatedPost.author}</p>
                  <p style={{ fontSize: 12, color: '#8a96a8' }}>{t('blogPost.expert')}</p>
                </div>
              )}
              ...
              <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: '18px 20px', border: '1px solid #e8edf4', boxShadow: '0 2px 10px rgba(15,31,61,0.05)' }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: '#8a96a8', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 14 }}>{t('blogPost.articleInfo')}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[
                    { icon: <Calendar size={14} color="#FF6B35" />, label: t('blogPost.published'), value: translatedPost.date },
                    { icon: <Clock size={14} color="#FF6B35" />, label: t('blogPost.readTime'), value: translatedPost.readTime },
                    { icon: <Eye size={14} color="#FF6B35" />, label: t('blogPost.views'), value: (translatedPost.views || 0).toLocaleString() },
                    { icon: <MessageCircle size={14} color="#FF6B35" />, label: t('blogPost.comments'), value: translatedComments.length },
              ...
              <div style={{ background: 'linear-gradient(135deg, #0f1f3d 0%, #1B2E4B 100%)', borderRadius: 14, padding: '22px 20px', overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  <h4 style={{ color: '#fff', fontSize: 15, fontWeight: 800, marginBottom: 8 }}>{t('blogPost.interestedMachines')}</h4>
                  <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 12.5, lineHeight: 1.6, marginBottom: 16 }}>{t('blogPost.bookDemo')}</p>
                  <button
                  ...
                  >
                    {t('blogPost.requestDemo')} <ArrowRight size={14} />
                  </button>
                </div>
              </div>
